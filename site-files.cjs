const assetFiles = [
  ...
  ...['dm-sans', 'space-grotesk'].flatMap((family) => [400, 500, 600, 700].map((weight) => `${family}-latin-${weight}-normal.woff2`)),
].map((file) => `assets/${file}`);

const siteFiles = ['index.html', 'styles.css', 'engine.js', 'content.js', 'scenarios.js', 'questions.js', 'audio.js', 'app.js', ...assetFiles];
const publicFiles = [
